...
namespace Vectorized
{
	public static class QuatPerfTest
	{
		public static void DoTest()
		{
			...
			Console.WriteLine("Vectorized.Quaternion Performance Tests");
			...
			Console.WriteLine("Vectorized.Quaternion Perfomance test complete.\n");

		}
		public static void MultPerf(ref Quaternion[] qarray, ref Quaternion addRot)
		{
			Quaternion[] qout = new Quaternion[qarray.Length];
			Console.Write("Vectorized.Quaternion.op_multiply Iterations: {0} : ",1000000 * qarray.Length);
		...
		{
			Console.Write("Vectorized.Quaternion.Multiply    Iterations: {0} : ",1000000 * qarray.Length);
			Quaternion[] qout = new Quaternion[qarray.Length];
			var timestart = Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency;
			for (int t = 0; t < 1000000; t++)
			{
		...
		{
			Quaternion qout;
			Console.Write("Vectorized.Quaternion.Slerp       Iterations: 100000000 : ");
			var timestart = Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency;
		...
		public static void NormalizePerf()
		{
			...
			Quaternion output;
			Console.Write("Vectorized.Quaternion.Normalize   Iterations: 100000000 : ");
			var timestart = Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency;
		...
		{
			Console.Write("Vectorized.Quaternion.Concatenate Iterations: 100000000 : ");
